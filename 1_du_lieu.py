...
class Database:
    def __init__(self, db):
        ...
        self.cur.execute(
            "CREATE TABLE IF NOT EXISTS sinh_vien (id INTEGER PRIMARY KEY, masv text, ten text, lop text, ngay_sinh text)")
        self.conn.commit()
    ...
    def insert(self, masv,ten,lop,ngay_sinh):
        self.cur.execute("INSERT INTO sinh_vien VALUES (NULL, ?, ?, ?, ?)",
                         (masv,ten, lop,ngay_sinh))
        self.conn.commit()
    ...
    def remove2(self, masv):
        ...
        self.conn.commit()    
    def update(self, id, masv,ten,lop,ngay_sinh):
        self.cur.execute("UPDATE sinh_vien SET masv = ?, ten = ?, lop = ?, ngay_sinh = ? WHERE id = ?",
                         (masv,ten,lop,ngay_sinh, id))
        self.conn.commit()
...
def add_sinh_vien():
    if ten_text.get() == '' or masv_text.get() == '' or lop_text.get() == '' or ngay_sinh_text.get() == '':
    ...
    db.insert(masv_text.get(), ten_text.get(),
              lop_text.get(), ngay_sinh_text.get())
...
def select_router(event):
    try:
        ...
        ngay_sinh_entry.insert(END, selected_item[4])
    except IndexError:
        pass
...
# ngaysinh
ngay_sinh_text = StringVar()
ngay_sinh_label = Label(frame_fields, text='Ngày Sinh', font=('bold', 12), pady=20)
ngay_sinh_label.grid(row=1, column=2, sticky=E)
ngay_sinh_entry = Entry(frame_fields, textvariable=ngay_sinh_text)
ngay_sinh_entry.grid(row=1, column=3, sticky=W)
...
columns = ['id', 'Mã SV', 'Tên', 'Lớp', 'Ngày sinh']
